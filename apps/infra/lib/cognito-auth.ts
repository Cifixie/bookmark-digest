 ...
 * lifetime.
 *
 * Config (from `apps/infra/outputs.json`, regenerated by `pnpm outputs`):
 *   CliUserPoolClientId   CLI client id (cdk output)
...
import { cliClientId, loadStackOutputs, stackRegion } from "./stack-outputs";
...
export function loadCognitoConfig(): CognitoConfig {
  ...
  // Env vars remain as explicit overrides (e.g. another account's CLI client),
  // but the stack outputs are the default so no env needs injecting by hand.
  return {
    clientId: process.env.BKDG_CLIENT_ID ?? cliClientId(outputs),
    region: process.env.BKDG_REGION ?? stackRegion(outputs),
  };
}
